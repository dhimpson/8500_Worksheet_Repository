...
**3) Geometry:** The type or shape of the visual elementsq on the graph. For example, this could be a point in the case of a scatter plot, a bar in the case of a bar plot, or a line in the case of a line plot.
...
```{r}
gg_religion_time <- gayguides %>% filter(grepl("religious|religious institution", type, ignore.case = TRUE)) %>% group_by(Year) %>% summarize(count = n())
ggplot(gg_religion_time, aes(x = Year, y = count))+
  geom_line()
```
...
```{r}
gg_risks <- gayguides %>% filter(grepl("HOT|AYOR|inquire locally", amenityfeatures)) %>% group_by(Year) %>% summarize(count = n())
## need to add where address has "inquire locally". Year on x, coutn on side, use line graph... do  Hot| AYOR first then do an second grepl for aminty features.
ggplot(gg_risks, aes(x= Year, y= count))+
  geom_line()
```
...
```{r}
SC_rec_spending <- rec %>% filter(state == "SC") %>% group_by(city) %>% summarize(total_exp = sum(total_expenditures))

ggplot(SC_rec_spending, aes(x = city, y = total_exp)) +
...
```{r}
gg_local_types <- gayguides %>%filter(between(Year, 1965,1980)) %>%
 filter(type %in% c("Cruising Areas","Book Store","Bars/Clubs", "Book Store", "Hotel")) %>% 
 group_by(type) %>%
 summarize(count = n())

ggplot(gg_local_types, aes(x = type, y = count)) +
  geom_col() +
  facet_wrap(~type)
```
...
colnames(rec.workers) <- gsub("X", "", colnames(rec.workers))
## As noted in class, I used gsub() to get rid of X in the year columns first.
rec.workers.long <- rec.workers %>% 
  pivot_longer(cols = -c(city, type_of_worker), names_to = "year", values_to = "workers")
## used pivot_longer() so that the wide year format were converted into a long format that should work better for ggplot. 
avg_workers <- rec.workers.long %>% group_by(year, type_of_worker) %>% summarize(avg_workers = mean(workers, na.rm = TRUE)) ## this calculates average workers by year and by type (not sure this step is correct)

ggplot(avg_workers, aes(x = year, y = avg_workers, color = type_of_worker, group = type_of_worker)) + 
geom_line () + 
geom_point() + 
labs(x = "Year", y = "Average Number of Workers", 
       title = "Average Volunteer and Paid Workers Over Time",
       color = "Worker Type")
```
## I had a lot of help from Claude with this one. I do not like the visualization and I think something went wrong somewhere, but I do not know where. I made notes as I went because I kept getting lost in the process. 
...
(@) Plot 1
```{r}
Gville_gg_locations <- gayguides %>% mutate(decade = floor(Year / 10) * 10) %>% filter(state == "SC", city %in% c("Greenville",  "Spartanburg")) %>% group_by(decade) %>% summarize(count = n())

ggplot(Gville_gg_locations, aes(x = decade, y = count)) +
  geom_col()+
  labs(x = "Decade", y = "Number of Locations", title = "Gay Guides Locations in Greenville and Spartanburg")
```

(@) Plot 1 explanation: 
> I wanted to explore the total number of locations in the gayguides data for Greenville, South Carolina by decade. My original thought was to see if the location totals dropped as church denominations like Southern Baptists pushed an anti-gay agenda within the upstate of South Carolina through the 1980s and 1990s. I decided to include Spartanburg in the data because of its proximity to Greenville and its similar religious culture. My bar chart shows that the number of locations actually grew through the 1980s, in spite of an increase in anti-gay rhetoric from religious institutions in these two cities.

(@) Plot 2
```{r}
data(charlestonManumissions)
top_motives <- charlestonManumissions %>% group_by(Motive) %>% summarize(count = n()) %>% arrange(desc(count)) %>% head(3)

ggplot(top_motives, aes(x = Motive, y = count)) +
  geom_col() +
  labs(x = "Motive for Manumission", y = "Number of Manumissions", title = "Top Motives for Manumission in Charleston")
```

(@) Plot 2 explanation: 
> The Charleston Manumissions dataset presents interesting information about enslaved people who gained freedom. I noticed that the Motive column contained many different reasons for obtaining freedom. I wanted to know what the top three reasons were and this code (and chart) presents that information clearly. 
...
```{r}
judges_race_birth <- judges %>% filter(Race.or.Ethnicity != "White", !is.na(Race.or.Ethnicity), !is.na(Gender))

ggplot(judges_race_birth, aes(x = Race.or.Ethnicity, fill = Gender)) +
  geom_bar(position = "dodge") +
  labs(x = "Race/Ethnicity", y = "Count", 
       title = "Non-White Judges by Race/Ethnicity and Gender") + 
  coord_flip()
```

(@) Plot 3 explanation: 
> I wanted to use the judges dataset to see what a side by side comparison would look like of minority judges by gender. This chart provides a glimpse of this comparison and shows that the highest amount of non-white judges for both genders was African Americans, followed by Hispanic and Asian Americans.
