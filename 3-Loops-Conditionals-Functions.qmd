...
```{r}
if (enrollment <= 30) print (TRUE) else print("not yet")
```
...
```{r}
list_of_presidents <- c("Carter", "Regan","HWBush","Clinton", "WBush","Obama")
if (length(list_of_presidents > 5)) {
  print(TRUE) 
} else {
  print("add more presidents")
}
```
...
```{r}
if (any(gayguides$Year > 1990)) {
  print("The dataset contains years after 1990.")
} else {
  print("No years after 1990 were found.")
}
```
...
```{r}
classes <- c("History 8500", "History 8520")
...
```
### check this again since I added ignore case
...
### The first and second time the loop runs it notes that the value is TRUE for 'i<3' and it moves onto the 'next' and skips 1 and 2. The third time it notes that i is not less than 3 and it is not greater than or equal to 5 so it prints the number 3. The same thing happens for 4. The fifth run sees i meeting the condition of greater than or equal to 5 and the 'break' is intiated and ends the loop.
...
```
### not sure if i did this right? my first attempt, i got [i] 5. i googled it and saw that i might need to tell R to look at the content of the city column, not just the number of entries. However, my new code says 'location cannot be verified. General city or location coordinates used.' Fixed by spelling length correctly... sheesh.
...
- Use `bTreak` to stop the loop after finding 5 locations (no need to search all 60k rows!)
...
```{r}
## I do not think that I can run this function.
```
...
count_chars <- function(text) {
  number <- nchar(text)
  result <- paste("There are", number, "characters in that string.")
  return(result)
}
count_chars("stressedout")
```

(@) Reflect on the process of building the above function. How did you go about figuring this out? 

>I began by creating my function and telling r that my input will be a piece of text.  I then loaded the function into my global enviroment to use later. I then created a code to look at the string and count the characters in that string. This amount will be saved to a variable called number. I asked the program to give me a result that would paste together the words in my desired output statement and include the number of characters in whatever string the function examines. The result for my test of my function count_chars("stressedout") was "There are 11 characters in that string." 
...
>## Is it that the computer does not know who he is? While the grade() function is loaded, there is no data relating George Washington to this function.
...
```{r}
gg.locations_by_state <- function (d.state, d.year) {
  data(gayguides)
  for (i in 1:length(gayguides$state)) {
    if (gayguides$state[i] == d.state && gayguides$Year[i] == d.year) {
      print(paste("Found a location in [d.state] called [location title]", gayguides$title[i]))
    } else {
      next
    }
  }
}
gg.locations_by_state("SC", 1975)
```
...
bwv.occupations <- function(job_title) { 
  data(BostonWomenVoters)
  result <- data.frame()
  for (i in 1:nrow(BostonWomenVoters)) {
    if (BostonWomenVoters$Occupation[i] == job_title)
  {
...
bwv.occupations("Housewife")
bwv.occupations.result <- bwv.occupations("Housewife")
test_results <- bwv.occupations("Housewife")
...
count_by_state <- function(st_abbrev) {
  ...
  for (i in 1:nrow(gayguides)) {
    ...
    if (!is.na(gayguides$state[i]) && gayguides$state[i] == st_abbrev) {
      count <- count + 1
    }
  }
  statement <- paste("There are", count, "locations in", st_abbrev)
  return(statement)
}

count_by_state("CA")
##"There are 12467 locations in CA"
...
```{r}
## start here and follow pattern for d.year and d.city
## come back to this later. 

count_by_state_and_year <- function(st_abbrev, target_year)  {
  count <- 0
  for (i in 1:nrow(gayguides)) {
   
    if (!is.na(gayguides$state[i]) && gayguides$state[i] == st_abbrev && gayguides$Year == target_year) {
      count <- count + 1
    }
  }
  statement <- paste("There are", count, "locations in", st_abbrev)
...
count_by_state_and_year("SC",1975)
```
...
>## A loop is code written to repeat a series of actions over a vector. Conditionals such as if, else, and next provide options where the code can diverge from the loop's set path to perform another function if the conditon is met. A function is a specific set of code that is written for the computer execute a action in a specific way. Functions are stored in the enviroment of Visual Studio Code and can be called upon when needed to execute. These items work in tandem to allow the user to sort through data and specify what data needs to be found in a dataset. The question at line 400 contains a loop which searches through the gayguides data for numerical locations in a specified state. It uses the function count_by_state <- function(st_abbrev) to do this and has conditionals that result in either desired results returned or ignored based upon if the data meets the conditions that are given.
