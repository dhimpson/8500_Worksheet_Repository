...
tidy_twc <- tidy_twc %>% filter(!grepl('[0-9]', word))
#tidy the dataset
```
...
#First I created a document with words that could be counted. I am a little unsure about the reasoning behind this step.
twc.dtm <- tidy_twc_words %>% 
  cast_dtm(filename, word, n)
#Next I used cast_dtm() to create a documnet-term matrix.
```

```{r}
twc.lda <- LDA(twc.dtm, k = 10, control = list(seed = 12345))
twc.lda
