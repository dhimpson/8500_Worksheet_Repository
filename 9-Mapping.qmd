...
```{r}
leaflet(data=test_coords) %>% addTiles() %>%
addMarkers(~longitude, ~latitude, popup = paste("The locations of sample voters in MA"))
```
...
> I kept things simple because my data was only a slice of the Boston Womens Voter data. Since the locations share a city and state, the popups were rather simplistic. I could see the value in using leaflet over ggplot when dealing with multiple locations across one city or state. It allows for a more interactive experience then the ggplot static maps.
...
```
```{r}
#FYI- I did not see an email come through about Dr. Barczewsk's project. I tried to see the documentation for this dataset in a hope to gain a better understanding of her project.?parks did not yield documentation. I decided to begin by creating a bar chart to visualize park by country.

unique(parks$country)  # This allowed me to see what countries are in the dataset

ggplot(parks, aes(x = country)) +
  geom_bar(fill = "steelblue", color = "black") +
  labs(title = "Number of National Parks by Country",
       x = "Country",
       y = "Count")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) #this last bit adjusts the text along the x axis to a slant for better readability.
#This shows me that Canada, Finland, Italy, Norway, Ukraine, and the United States all have more than 40 national parks.
```


```{r}
colnames(parks)
#noting that the columns I need are called "lat", "lon", and "park"

#attempt below is of a static map using ggplot
world <- map_data("world")

ggplot() + 
  geom_map(data = world, map = world, aes(long, lat, map_id = region), 
           fill = "lightgray", color = "black") +
  geom_point(data = parks, mapping = aes(x = lon, y = lat), 
             color = "red", size = 2, alpha = 0.6) +
  labs(title = "National Parks Around the World",
       x = "Longitude",
       y = "Latitude") +
  coord_fixed(1.3)
```
#Now I want to see if I can map just the countries with more than 40 national parks. I would like to create a map in leaflet that shows these parks.
```{r}
# First, identify which countries have more than 40 parks
countries_with_many_parks <- parks %>%
  group_by(country) %>%
  summarize(park_count = n()) %>%
  filter(park_count > 40) %>%
  pull(country)

# ^this yielded this data set: c("Canada", "Finland", "Norway", "Ukraine", "United States")

# next: Filter the parks data to only include those countries.
parks_filtered <- parks %>%
  filter(country %in% countries_with_many_parks)

# Map to leaflet
leaflet(data = parks_filtered) %>% 
  addTiles() %>%
  addMarkers(~lon, ~lat, 
             popup = paste("Park: ", parks_filtered$park, "<br>Country: ", parks_filtered$country, sep=""))
```

# I am not quite sure what argument Dr. Barczewsk is making with her data, but a few ways that this data could be used further would be to map the addition of parks over time throughout the world. I would look at this information in light of the rise and fall of nations or empires (ie. The British Empire or the fall of the USSR)? It might be also interesting to add population data for the nearest cities into the dataset and examine if these parks tended to be built near cities of sizable populations? Were the parks constructed as an escape from city life or to preserve the natural enviroment? 