...
>I saved the dataframe as a variable. I can then call it later to use in other arguments.
...
```{r}
gayguides.info <- gayguides %>% select(-city, -state)
```
...
```{r}
home_gayguides_in_seventies <- gayguides %>% 
  filter(city == "Greenville" & state == "SC") %>%
  filter(Year >= 1975 & Year <= 1980)
```
```
...
## cannot get this cell to run. Says it is not executable. Note that I ran it in the block below.

```{r}
locations_in_1975 <- gayguides %>% 
...
```
```{r}
a.gayguides <- gayguides %>%
   filter(grepl("(G)",gayguides$amenityfeatures) | grepl("L", gayguides$amenityfeatures))
```
...
```{r}
gayguides %>% mutate(city = str_to_title(city))
```
...
```{r}
b.gayguides <- gayguides %>% 
mutate(has_dancing = grepl("D", amenityfeatures, fixed = TRUE))
View(b.gayguides)
```

(@) In the Boston Women Voters data, occupation names may have extra whitespace. Use `str_trim()` to clean the occupation column.
```{r}
BostonWomenVoters %>% mutate(str_trim("Occupation"))
```
...
arrange(desc(X1850))
head(STATE)
``` ## New York had the largest population in 1850. 
...
```{r}
## There were 42 rows with missing information. I used nrow(BostonWomenVoters) to get a row count of the original dataset (10702)and then subtracted the rows that were removed from the previous code after cleaning (10660)and came up with 42.
```

(@) Why might ward information be missing for some voters? What does this tell us about the historical record?

> This information may be missing for numerous reasons. It could have been omited on the original record or the handwriting might have been unreadable on the original record resulting in errors in transcription. To rule these issues out, I would double check these 42 entries with the original sources,if possible. The fact that there are only 42 missing entries out of over 10,000 entries demonstrates that those who were in charge of these voting wards were meticulous in their job of recording voting data for women.
...
```{r}
BostonWomenVoters %>% 
  mutate(ward_missing = is.na(Ward))
...
gg_type <- gayguides %>%
  ...
  nlevels(gg_type$type)
 ``` ## I found 235 different types of locations
...
bwv_jobs <- BostonWomenVoters %>%
  ...
  nlevels(bwv_jobs$Occupation)
``` ## I modified the levels part of the question based off of your response to Sharon's question in slack. I found 1137 levels.
