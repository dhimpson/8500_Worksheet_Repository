...
```{r}
locations_in_1975 <- gayguides %>% filter(city != "New York" & city != "San Francisco") %>%
filter(Year == 1975)
```
...
```
```{r}
a.gayguides <- gayguides %>% filter(grepl("(G)",gayguides$amenityfeatures) | grepl("L", gayguides$amenityfeatures))
```
...
```{r}
b.gayguides <- gayguides %>% mutate(has_dancing = grepl("D", amenityfeatures, fixed = TRUE))
...
```{r}
largest_population <- statepopulations %>% 
  filter(year == 1850) %>%
