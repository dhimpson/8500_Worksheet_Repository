...
```{r}
paste("I love to run", "long distances")
```

(@) Can you explain what happened in 2-3 sentences? 

> Using the paste function, I was able to concatenate (join) two different strings of text into one string of text. This function combined two character vectors into one, single vector.
...
>line 94 reads 5 is equal to 10; computer renders it FALSE
line 95 reads 3 is less than 4; computer renders it TRUE
line 96 reads 5 is not equal to 10; computer renders it TRUE
line 97 reads 5 is not equal to 5; computer renders it FALSE
line 98 reads 3 is equal to 4 or 3; computer renders it TRUE
line 99 reads 3 is not equal to 4 and 3 is not equal to 5; Computer renders it TRUE
...
```{r}
year_to_check <- 1975
is_in_range <- (year_to_check >= 1970) & (year_to_check <= 1980)
print(is_in_range)
```
...
number_of_Impsons <- 6
number_of_forks <- 4
number_of_Impsons * number_of_forks
```
...
```
```{r}
paste("In 1492","Columbus sailed the ocean blue")
```
...
R took the vector of my numbers which contained numbers 2,4,6,8 and multiplied 2 across all the numbers within the value. This is R doing the distributive property of multiplication.
>
...
> R took the third value in the vector, which was 6, and multiplied it by 2 to get 12.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
my_family <- c("David", "Isaiah", "Selah","Joshua","Shylah")
```
...
?floor()
```
Floor takes a numeric argument and returns a numeric vector with the largest integers not greater than the corresponding elements of x.
...
>Line z take the first column, x, multiplies its values by 2 and then adds the values of line y, from the second column, to get a new answer
...
> It tells me the data is compiled from address books that span 1965 to 1985 and also the format of the data is available in one data frame with 14 variables and 60698 obeservations.
...
> I googled them and then tried what I found. 
...
> Gayguides is the specific dataset that we are using, $ is going to access only a specific part of this dataset-in this case just the title which is following it, the 500 is lookin at the 500th item that is in the title row within the dataset. 
...
```{r}
earliest_year <- min(gayguides$Year, na.rm = TRUE)
print(earliest_year)
```
#the answer i got was 1965. this took a lot of googling and i think i did it the hard way.
...
print(latest_year)
```
#the latest year was 1985. i got this by modifying my previous earliest year attempt.
...
#I ran the undergroundRR dataset. There are 997 observations and 18 variables.
...
```{r}
head(undergroundRR, 100)
```

(@) What did you learn about the dataset? What can you tell me about it? 

> I learned that the dataset contains details about individuals that Still processed through the underground RR. I found most interesting notes on how people were transported, who their enslaver was, and if they were literate. I attempted to get a mean for the age, but could not get the code to work. I think that finding the average age of those who were helped would be interesting- my gut says it would be in the lower thirties, but I would like to know for sure.
