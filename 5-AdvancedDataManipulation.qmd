...
dates_demo <- c("12/05/2020", "1997-06-06", "29-08-1967") %>%
  parse_date_time(c(dates_demo), "dmy")
``` 
...
```{r}
gayguides_ymd <- gayguides %>%
  mutate(Decade = (year(Date)%/% 10) *10) ## this seemed to create a new Decade column, rounded my dates to the nearest decade, and put that value in the decade column
gayguides_ymd <- gayguides %>%
  mutate(Decade = floor(Year /10) *10)
## It looks like I got the same result for both. 
```
...
```{r}
BostonWomenVoters_birthday <- BostonWomenVoters %>%
  mutate(BirthYear = 1920 - Age) %>%
  mutate(BirthDate = ymd(paste(BirthYear, "07-01",sep = "-")))
```
...
```{r} gayguides_decades <- gayguides %>%
ggint_60s <- interval(ymd("1960-01-01")), ymd("1969-12-31")
ggint_70s <- interval(ymd("1970-01-01")), ymd("1979-12-31")
ggint_80s <- interval(ymd("1980-01-01")), ymd("1989-12-31")

```##Not sure what to do for the second part of this question.
...
```{r}
January_Dates <- function(year_imput) {
  make_date(year = year_imput, month = 1, day =1)
}
January_Dates(1967)
January_Dates(1989)
January_Dates(1899)
...
> ## Dates with mismatched data (dmy and ymd) can be tidied to one standard date format, dates can also be rounded to nearest decade. These things help to make a messy data set more useful.
...
(@) First, tell me, what is the question you are asking? ## How many judges were women? 
```{r}
data("judges")
head(judges)
View(judges)
## How many judges were women? When were they confirmed?
```
...
```{r}
Female_Judges <- judges %>% ## First I set up a dataframe for Female Judges. This way I can check to see if the code actually works.
    filter(Gender == "Female") %>% ##used Filter to remove any male judges from the data.
    mutate(new_confirm = ymd(`Confirmation.Date..1`),
      Decade = (year(`Confirmation.Date..1`) %/% 10) *10) ## I keep getting errors re: Confirmation Date
```

(@) Now ask a question of the `gayguides` data (or another dataset of your choice). What is the question you are asking? 
```{r}
## How many locations were in the Bible Belt (AL,AR,GA,KY,LA,MS,NC,OK,SC,TN) in the 1980s?
```

(@) Now write some code to address that question. Comment the code with notes that explain your thinking as you go. Use functions like select(), filter(), etc to pair down your dataset and reshape it to address your question. 
```{r}
BB_states <- c("AL","AR","GA","KY","LA","MS","NC","OK","SC","TN") ##First, I created my Bible Belt states variable

Bible_Belt_locals <- gayguides %>%
  filter(state %in% BB_states, year >= 1980 & year<= 1989) %>% ##Filtered to get the states that I wanted and the decade of the 80s. This info is then sent to be grouped by state and year
  group_by(state, year) %>%
  summarize(location_count = n()), .groups ='drop'## dropped any empty values
  nrow(Bible_Belt_locals)## gives the count of 60698 locations in the Bible Belt states in the 1980s
```
...
```{r}
gg_filter <- function(target_year,target_state) {
  gayguides %>% 
  filter(Year == target_year, state == target_state) %>%
