...
>It returned a list of values for x but excluded the third and first number from the orginal vector.
...
> Each value in the vector for x was assigned a true or false value. Then when the function was ran that tested for true or false values with the vector, only the values that have been assigned the value of TRUE were returned, which were the first, third, and fourth numerical values. The second function asked that only values which were greater than 7 be returned, resulting in the numbers 15.40 25.00 8.35 383.00 and 10.20 to be returned.These are the values that are greater than seven and prove to be TRUE when tested.
...
The vector resulted in a NA which means Not Available. Is this because "d" was entered as a character and letters were numerical?
...
```{r}
evens <- seq(from = 2, to = 18, by = 2)
```
```{r}
my_matrix <- matrix(evens, nrow = 3)
```
```{r}
print(my_matrix)
```
...
```
```{r}
head(BostonWomenVoters[,c("Age","Place.of.Business")])
```
I looked at the BostonWomensVoters data to see the age and place of business for the first 5 entries. I wondered what jobs women voters might have in this timeperiod and how old these voters might be. 
...
>Provided the cities listed from row 100 to 110
...
```{r}
ca_locations <- gayguides$state=="CA"
```
```{r}
is_1970s <- gayguides$date >= "1970-01-01" & gayguides$date <= "1979-12-31"
```
```{r}
CA_70s <- is_1970s & ca_locations
```
...
```{r}
Women_Amenities_1975 <- subset(gayguides,Year == 1975 & grepl("(G)", amenityfeatures, fixed = TRUE))
nrow(Women_Amenities_1975)
```
# 250 locatiions catered to women that year.
...
str(BostonWomenVoters)
```{r}
sum(BostonWomenVoters$Age >= 50, na.rm = TRUE)
```
```{r}
sum(BostonWomenVoters$Age <= 50, na.rm = TRUE)
```
```{r}
Age_20_30 <- BostonWomenVoters [BostonWomenVoters$Age >= 20 & BostonWomenVoters$Age <= 30, ]
nrow(Age_20_30)
```
``` {r}
Age_31_40 <- BostonWomenVoters [BostonWomenVoters$Age >= 31 & BostonWomenVoters$Age <= 40, ]
nrow(Age_31_40)
```
```{r}
Age_41_50 <- BostonWomenVoters [BostonWomenVoters$Age >= 41 & BostonWomenVoters$Age <= 50, ]
nrow(Age_41_50)
```
## The total women over 50 years is 2583. The total women voters under 50 is 8294. I found out the 3187 of these women were in the 20 to 30 age bracket. 2795 were ages 31 to 40 and 2537 were between the ages of 41 and 50. I find it interesting that the largest group of women voters under 50 were between the ages of 20 and 30, just when their adult lives are beginning to come to solidify.
...
```{r}
Main_St_Voters <- BostonWomenVoters[grepl("Main", BostonWomenVoters$Street.of.Residence.on.April.1),]
nrow(Main_St_Voters)
```
## I came up with 4 voters who lived on "Main" street.

(@) Using subsetting techniques, can you find the occupations of the five oldest women in the dataset? What were they?
```{r}
BostonWomenVoters[order(BostonWomenVoters$Age, decreasing = TRUE), "Occupation"] [1:5]
```
### they are: [1] "At home"        "At home"        "[home?]"        "Housekeeper"   [5] "Widow\nRetired"
